...
class RateLimitPolicySpec extends AnyFlatSpec with Matchers {
  ...
  "delayBeforeNext" should "use the configured floor before anything is known" in {
    RateLimitPolicy.delayBeforeNext(None, 440L, now, Some(0L)) shouldBe 440L
  }
  ...
  it should "space reads to the 600-a-minute limit with a tenth in hand" in {
    RateLimitPolicy.delayBeforeNext(window(600, 590, 60), 0L, now, Some(0L)) shouldBe 110L
  }

  it should "space writes to the 150-a-minute limit with a tenth in hand" in {
    RateLimitPolicy.delayBeforeNext(window(150, 140, 60), 0L, now, Some(0L)) shouldBe 440L
  }

  it should "honour a floor the operator has raised" in {
    RateLimitPolicy.delayBeforeNext(window(600, 590, 60), 5000L, now, Some(0L)) shouldBe 5000L
  }

  it should "count the previous request's round trip toward the gap" in {
    val w = window(150, 140, 60) // 440 ms apart
    RateLimitPolicy.delayBeforeNext(w, 0L, now, Some(300L)) shouldBe 140L
    RateLimitPolicy.delayBeforeNext(w, 0L, now, Some(440L)) shouldBe 0L
    RateLimitPolicy.delayBeforeNext(w, 0L, now, Some(5000L)) shouldBe 0L
  }
  ...
  it should "wait for the window to reset when the allowance is nearly gone" in {
    ...
    RateLimitPolicy.delayBeforeNext(window(150, 10, 30), 0L, now, Some(0L)) shouldBe 31000L
  }
  ...
  it should "keep one request in reserve when a tenth of the limit rounds to nothing" in {
    ...
    RateLimitPolicy.delayBeforeNext(window(5, 2, 30), 0L, now, Some(0L)) should be < 31000L
  }

  it should "wait for the reset however long ago the last request went out" in {
    RateLimitPolicy.delayBeforeNext(window(150, 10, 30), 0L, now, Some(5000L)) shouldBe
    31000L
  }

  it should "never wait longer than a window and a half, however odd the reset time" in {
    RateLimitPolicy.delayBeforeNext(window(150, 0, 86400), 0L, now, Some(0L)) shouldBe 90000L
  }

  it should "not wait on a reset time that has already passed" in {
    RateLimitPolicy.delayBeforeNext(window(150, 0, -120), 440L, now, Some(0L)) shouldBe 440L
  }

  it should "treat a limit of zero as no information" in {
    RateLimitPolicy.delayBeforeNext(window(0, 0, 30), 440L, now, Some(0L)) shouldBe 440L
  }

  "latest" should "take the first window seen" in {
    RateLimitPolicy.latest(None, RateLimitWindow(150, 140, nowEpoch)) shouldBe
    RateLimitWindow(150, 140, nowEpoch)
  }

  it should "ignore a window that resets before the one already seen" in {
    val current = RateLimitWindow(150, 5, nowEpoch + 60)
    RateLimitPolicy.latest(Some(current), RateLimitWindow(150, 140, nowEpoch)) shouldBe current
  }

  it should "keep the lower remaining count within one window" in {
    val current = RateLimitWindow(150, 5, nowEpoch + 60)
    RateLimitPolicy.latest(Some(current), RateLimitWindow(150, 140, nowEpoch + 60)) shouldBe
    current
    RateLimitPolicy.latest(Some(current), RateLimitWindow(150, 2, nowEpoch + 60)) shouldBe
    RateLimitWindow(150, 2, nowEpoch + 60)
  }

  it should "replace the window once a newer one resets later" in {
    val current = RateLimitWindow(150, 5, nowEpoch + 60)
    RateLimitPolicy.latest(Some(current), RateLimitWindow(150, 140, nowEpoch + 120)) shouldBe
    RateLimitWindow(150, 140, nowEpoch + 120)
  }
