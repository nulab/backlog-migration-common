...
/**
  * @author uchida
  */
class BacklogPaths(projectKey: String, basePath: Path = Paths.get("./backlog")) {
  ...
  def projectUsersJson: BetterFile = outputPath / "project" / projectKey / "projectUsers.json"

  def customFieldSettingsJson: BetterFile = outputPath / "project" / projectKey / "customFieldSettings.json"

  def issueCategoriesJson: BetterFile = outputPath / "project" / projectKey / "issueCategories.json"

  def issueTypesJson: BetterFile = outputPath / "project" / projectKey / "issueTypes.json"
  ...
  def wikiAttachmentDirectoryPath(directory: String): BetterFile = wikiDirectoryPath / FileUtil.clean(directory) / "attachment"
  ...
  def issueDirectoryPath: BetterFile = outputPath / "project" / projectKey / "issues"

  def issueDirectoryPath(eventType: String, issueId: Long, created: Date, index: Int): BetterFile =
  ...
  def issueAttachmentDirectoryPath(path: BetterFile): BetterFile = path / "attachment"

  def issueAttachmentPath(path: BetterFile, fileName: String): BetterFile = path / FileUtil.clean(fileName)
