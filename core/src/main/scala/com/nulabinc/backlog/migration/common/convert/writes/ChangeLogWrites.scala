...
private[common] class ChangeLogWrites @Inject() (
  ...
  override def writes(changeLog: ChangeLog): BacklogChangeLog = {
    BacklogChangeLog(
      ...
      optNewValue = Option(changeLog.getNewValue).map(formatNewValue(changeLog.getField)),
  ...
  private[this] def formatNewValue(field: String): String => String =
    if (field == "attachment") FileUtil.clean
    else DateUtil.formatIfNeeded
