...
object MappingSerializer {
  ...
  def status[A](mappings: Seq[StatusMapping[A]])
               (implicit serializer: Serializer[StatusMapping[A], Seq[String]],
                header: MappingHeader[StatusMapping[_]]): Observable[Array[Byte]] =
    fromHeader(header) +: toObservable(mappings)

  def priority[A](mappings: Seq[PriorityMapping[A]])
                 (implicit serializer: Serializer[PriorityMapping[A], Seq[String]],
                  header: MappingHeader[PriorityMapping[_]]): Observable[Array[Byte]] =
    fromHeader(header) +: toObservable(mappings)

  def user[A](mappings: Seq[UserMapping[A]])
             (implicit serializer: Serializer[UserMapping[A], Seq[String]],
              header: MappingHeader[UserMapping[_]]): Observable[Array[Byte]] =
    fromHeader(header) +: toObservable(mappings)
  ...
  private def toObservable[A](items: Seq[A])(implicit serializer: Serializer[A, Seq[String]]): Observable[Array[Byte]] =
    Observable
